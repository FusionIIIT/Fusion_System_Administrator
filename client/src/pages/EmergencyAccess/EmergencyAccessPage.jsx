                ...
                <Grid.Col span={4} key={role.id}>
                  ...
                  >
                    <Group position="apart" mb="xs">
                      ...
                      <Badge color="orange" leftSection={<IconShield size={12} />}>TEMPORARY</Badge>
                    </Group>
                    <Group spacing={4} mb="xs">
                      <IconClock size={14} />
                      <Text size="sm" weight={600} color="orange.7">
                        ⏱️ Expires in: {getTimeRemaining(role.expires_at)}
                      </Text>
                    </Group>
                    <Text size="xs" color="dimmed">
                      Expiry: {new Date(role.expires_at).toLocaleString()}
                    </Text>
                    {role.time_remaining_minutes && (
                      <Text size="xs" color="dimmed" mt={4}>
                        Remaining: {Math.floor(role.time_remaining_minutes / 60)}h {role.time_remaining_minutes % 60}m
                      </Text>
                    )}
                  </Card>
                </Grid.Col>
        ...
        <Stack spacing="lg">
          {detailModal.request && (
            <>
              <Grid>
              <Grid.Col span={6}>
                <Text size="xs" color="dimmed">Requester</Text>
                <Group spacing="sm">
                  <Text weight={500}>{detailModal.request.user?.username || 'N/A'}</Text>
                  {detailModal.request.user?.email && (
                    <Text size="sm" color="dimmed">({detailModal.request.user.email})</Text>
                  )}
                </Group>
              </Grid.Col>
              <Grid.Col span={6}>
                <Text size="xs" color="dimmed">Role</Text>
                <Text weight={500}>{detailModal.request.role?.full_name || detailModal.request.role?.name || 'N/A'}</Text>
              </Grid.Col>
            </Grid>
            ...
            <Stack spacing="md">
              <Group grow>
                <div style={{ flex: 1 }}>
                  <Text size="xs" color="dimmed">Reason for Request</Text>
                  <Text>{detailModal.request.reason || 'N/A'}</Text>
                </div>
              </Group>
              ...
              <Group grow>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <Text>{detailModal.request.approved_duration ? formatDuration(detailModal.request.approved_duration) : 'Not yet approved'}</Text>
                </div>
              </Group>
              ...
              <Group grow>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <Text>{detailModal.request.requested_at ? new Date(detailModal.request.requested_at).toLocaleString() : 'N/A'}</Text>
                </div>
                <div style={{ flex: 1 }}>
                  <Text size="xs" color="dimmed">Reviewed At</Text>
                  <Text>{detailModal.request.reviewed_at ? new Date(detailModal.request.reviewed_at).toLocaleString() : 'Not reviewed'}</Text>
                </div>
              </Group>
              ...
              <Group grow>
                <div style={{ flex: 1 }}>
                  ...
                  <Text>{detailModal.request.reviewed_by?.username || 'Not reviewed'}</Text>
                </div>
                <div style={{ flex: 1 }}>
                  <Text size="xs" color="dimmed">Expires At</Text>
                  <Text>{detailModal.request.expires_at ? new Date(detailModal.request.expires_at).toLocaleString() : 'Not set'}</Text>
                </div>
              </Group>
